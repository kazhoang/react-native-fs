...
var RNFSManagerCustomize = require('react-native').NativeModules.RNFSManagerCustomize;

var NativeEventEmitter = require('react-native').NativeEventEmitter;

var RNFS_NativeEventEmitter = new NativeEventEmitter(RNFSManagerCustomize);

var base64 = require('base-64');
var utf8 = require('utf8');
var isIOS = require('react-native').Platform.OS === 'ios';

var RNFSFileTypeRegular = RNFSManagerCustomize.RNFSFileTypeRegular;
var RNFSFileTypeDirectory = RNFSManagerCustomize.RNFSFileTypeDirectory;
...
var RNFS = {
  ...
  mkdir(filepath: string, options: MkdirOptions = {}): Promise<void> {
    return RNFSManagerCustomize.mkdir(normalizeFilePath(filepath), options).then(() => void 0);
  },

  moveFile(filepath: string, destPath: string, options: FileOptions = {}): Promise<void> {
    return RNFSManagerCustomize.moveFile(normalizeFilePath(filepath), normalizeFilePath(destPath), options).then(() => void 0);
  },

  copyFile(filepath: string, destPath: string, options: FileOptions = {}): Promise<void> {
    return RNFSManagerCustomize.copyFile(normalizeFilePath(filepath), normalizeFilePath(destPath), options).then(() => void 0);
  },

  pathForBundle(bundleNamed: string): Promise<string> {
    return RNFSManagerCustomize.pathForBundle(bundleNamed);
  },

  pathForGroup(groupName: string): Promise<string> {
    return RNFSManagerCustomize.pathForGroup(groupName);
  },

  getFSInfo(): Promise<FSInfoResult> {
    return RNFSManagerCustomize.getFSInfo();
  },

  getAllExternalFilesDirs(): Promise<string> {
    return RNFSManagerCustomize.getAllExternalFilesDirs();
  },

  unlink(filepath: string): Promise<void> {
    return RNFSManagerCustomize.unlink(normalizeFilePath(filepath)).then(() => void 0);
  },

  exists(filepath: string): Promise<boolean> {
    return RNFSManagerCustomize.exists(normalizeFilePath(filepath));
  },

  stopDownload(jobId: number): void {
    RNFSManagerCustomize.stopDownload(jobId);
  },

  resumeDownload(jobId: number): void {
    RNFSManagerCustomize.resumeDownload(jobId);
  },

  isResumable(jobId: number): Promise<bool> {
    return RNFSManagerCustomize.isResumable(jobId);
  },

  stopUpload(jobId: number): void {
    RNFSManagerCustomize.stopUpload(jobId);
  },

  completeHandlerIOS(jobId: number): void {
    return RNFSManagerCustomize.completeHandlerIOS(jobId);
  },

  readDir(dirpath: string): Promise<ReadDirItem[]> {
    return readDirGeneric(dirpath, RNFSManagerCustomize.readDir);
  },

  // Android-only
  readDirAssets(dirpath: string): Promise<ReadDirItem[]> {
    if (!RNFSManagerCustomize.readDirAssets) {
      throw new Error('readDirAssets is not available on this platform');
    }
    return readDirGeneric(dirpath, RNFSManagerCustomize.readDirAssets);
  },

  // Android-only
  existsAssets(filepath: string) {
    if (!RNFSManagerCustomize.existsAssets) {
      throw new Error('existsAssets is not available on this platform');
    }
    return RNFSManagerCustomize.existsAssets(filepath);
  },

  // Android-only
  existsRes(filename: string) {
    if (!RNFSManagerCustomize.existsRes) {
      throw new Error('existsRes is not available on this platform');
    }
    return RNFSManagerCustomize.existsRes(filename);
  },
  ...
  setReadable(filepath: string, readable: boolean, ownerOnly: boolean): Promise<boolean> {
    return RNFSManagerCustomize.setReadable(filepath, readable, ownerOnly).then((result) => {
      return result;
    })
  },

  stat(filepath: string): Promise<StatResult> {
    return RNFSManagerCustomize.stat(normalizeFilePath(filepath)).then((result) => {
      return {
        'path': filepath,
  ...
  readFile(filepath: string, encodingOrOptions?: any): Promise<string> {
    return readFileGeneric(filepath, encodingOrOptions, RNFSManagerCustomize.readFile);
  },
  ...
  read(filepath: string, length: number = 0, position: number = 0, encodingOrOptions?: any): Promise<string> {
    ...
    return RNFSManagerCustomize.read(normalizeFilePath(filepath), length, position).then((b64) => {
      var contents;
  ...
  readFileAssets(filepath: string, encodingOrOptions?: any): Promise<string> {
    if (!RNFSManagerCustomize.readFileAssets) {
      throw new Error('readFileAssets is not available on this platform');
    }
    return readFileGeneric(filepath, encodingOrOptions, RNFSManagerCustomize.readFileAssets);
  },

  // Android only
  readFileRes(filename: string, encodingOrOptions?: any): Promise<string> {
    if (!RNFSManagerCustomize.readFileRes) {
      throw new Error('readFileRes is not available on this platform');
    }
    return readFileGeneric(filename, encodingOrOptions, RNFSManagerCustomize.readFileRes);
  },

  hash(filepath: string, algorithm: string): Promise<string> {
    return RNFSManagerCustomize.hash(normalizeFilePath(filepath), algorithm);
  },

  // Android only
  copyFileAssets(filepath: string, destPath: string) {
    if (!RNFSManagerCustomize.copyFileAssets) {
      throw new Error('copyFileAssets is not available on this platform');
    }
    return RNFSManagerCustomize.copyFileAssets(normalizeFilePath(filepath), normalizeFilePath(destPath)).then(() => void 0);
  },

  // Android only
  copyFileRes(filename: string, destPath: string) {
    if (!RNFSManagerCustomize.copyFileRes) {
      throw new Error('copyFileRes is not available on this platform');
    }
    return RNFSManagerCustomize.copyFileRes(filename, normalizeFilePath(destPath)).then(() => void 0);
  },
  ...
  copyAssetsFileIOS(imageUri: string, destPath: string, width: number, height: number,
    ...
    return RNFSManagerCustomize.copyAssetsFileIOS(imageUri, destPath, width, height, scale, compression, resizeMode);
  },
  ...
  copyAssetsVideoIOS(imageUri: string, destPath: string): Promise<string> {
    return RNFSManagerCustomize.copyAssetsVideoIOS(imageUri, destPath);
  },
  ...
  writeFile(filepath: string, contents: string, encodingOrOptions?: any): Promise<void> {
    ...
    return RNFSManagerCustomize.writeFile(normalizeFilePath(filepath), b64, options).then(() => void 0);
  },
  ...
  appendFile(filepath: string, contents: string, encodingOrOptions?: any): Promise<void> {
    ...
    return RNFSManagerCustomize.appendFile(normalizeFilePath(filepath), b64);
  },
  ...
  write(filepath: string, contents: string, position?: number, encodingOrOptions?: any): Promise<void> {
    ...
    return RNFSManagerCustomize.write(normalizeFilePath(filepath), b64, position).then(() => void 0);
  },
  ...
  downloadFile(options: DownloadFileOptions): { jobId: number, promise: Promise<DownloadResult> } {
    ...
    return {
      jobId,
      promise: RNFSManagerCustomize.downloadFile(bridgeOptions).then(res => {
  ...
  uploadFiles(options: UploadFileOptions): { jobId: number, promise: Promise<UploadResult> } {
    if (!RNFSManagerCustomize.uploadFiles) {
      return {
        jobId: -1,
    ...
    return {
      jobId,
      promise: RNFSManagerCustomize.uploadFiles(bridgeOptions).then(res => {
        subscriptions.forEach(sub => sub.remove());
        return res;
      })
    };
  ...
  touch(filepath: string, mtime?: Date, ctime?: Date): Promise<void> {
    ...
    return RNFSManagerCustomize.touch(
  ...
  scanFile(path: string): Promise<ReadDirItem[]> {
    return RNFSManagerCustomize.scanFile(path);
  },

  MainBundlePath: RNFSManagerCustomize.RNFSMainBundlePath,
  CachesDirectoryPath: RNFSManagerCustomize.RNFSCachesDirectoryPath,
  ExternalCachesDirectoryPath: RNFSManagerCustomize.RNFSExternalCachesDirectoryPath,
  DocumentDirectoryPath: RNFSManagerCustomize.RNFSDocumentDirectoryPath,
  DownloadDirectoryPath: RNFSManagerCustomize.RNFSDownloadDirectoryPath,
  ExternalDirectoryPath: RNFSManagerCustomize.RNFSExternalDirectoryPath,
  ExternalStorageDirectoryPath: RNFSManagerCustomize.RNFSExternalStorageDirectoryPath,
  TemporaryDirectoryPath: RNFSManagerCustomize.RNFSTemporaryDirectoryPath,
  LibraryDirectoryPath: RNFSManagerCustomize.RNFSLibraryDirectoryPath,
  PicturesDirectoryPath: RNFSManagerCustomize.RNFSPicturesDirectoryPath,
  FileProtectionKeys: RNFSManagerCustomize.RNFSFileProtectionKeys
};
